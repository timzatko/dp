...
class HeatmapEvaluationV3:
    ...
    def evaluate(self, log=False, verbose=0, seed=None, max_evaluations=None):
        """
        Evaluate sequence with provided method.
        :param max_evaluations: maximum number of evaluated images
        :param log:
        ...
        for batch_x, batch_y, *_ in self.sequence:
            ...
            for i, (image_x, image_y, y_pred) in enumerate(zip(batch_x, batch_y, batch_y_pred)):
                ...
                auc_ins = evaluation_auc(image_y, y_pred_heatmap, eval_seq.step_size)
                ...
                auc_del = evaluation_auc(image_y, y_pred_heatmap, eval_seq.step_size)
                ...
                arr_y_pred_heatmap.append(y_pred_heatmap)
                arr_y_pred.append(y_pred)
                arr_auc_ins.append(auc_ins)
                arr_auc_del.append(auc_del)
                arr_voxels.append(len(eval_seq.voxels))
                arr_max_voxels.append(eval_seq.max_voxels)
                arr_step_size.append(eval_seq.step_size)
                ...
                if log:
                    print(f'\nauc_ins: {auc_ins}')
                    print(f'auc_del: {auc_del}\n({str(datetime.timedelta(seconds=int(end - start)))}s)\n')

        auc_ins = sum(arr_auc_ins) / evaluations
        auc_del = sum(arr_auc_del) / evaluations
                    
        return HeatmapEvaluationHistory('insertion', auc_ins, arr_auc_ins, arr_heatmap, arr_x, arr_y, arr_y_pred,
                                        arr_y_pred_heatmap, arr_voxels, arr_max_voxels, arr_step_size), HeatmapEvaluationHistory('deletion', auc_del, arr_auc_del, arr_heatmap, arr_x, arr_y, arr_y_pred,
                                        arr_y_pred_heatmap, arr_voxels, arr_max_voxels, arr_step_size)
