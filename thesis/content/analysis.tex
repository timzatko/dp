...
\begin{itemize}
    ...
    \item Na tento výsledok použijeme funkciu \textit{ReLU} čím odstránime z tepelenej mapy časti obrázku, ktoré prispevajú proti predikovanej triede negatívne.
    \item Túto maticu pomocou bilineárnej interpolácie zväčšíme na veľkosť vstupu.
\end{itemize}
...
\subsubsection{Riadený GradCAM (angl. Guided GradCAM) \label{sec:guided_grad_cam}}

Guided GradCAM kombinuje metódu riadenej spätnej propagácie s metódou GradCAM. Výsledné tepelné mapy z oboch metód, pre vstupný obraz a vybranú triedu sú navzájom vynásobené (nejedná sa o maticové násobenie ale násobenie po prvkoch) čím vznikne nová tepelná mapa \cite{selvaraju2017grad} (Obr. \ref{fig:guided_grad_cam}). 

\begin{figure}[h!]
    \centering
    \includegraphics[scale=0.40]{assets/images/grad_cam_and_guided_grad_cam.png}
    \caption{Porovnanie metód Guided Backprop (b), GradCAM (c) a Guided GradCAM (d) \cite{selvaraju2017grad}. Tepelná mapa (d) je výsledkom násobenia medzi tepelnými mapami (b) a (c), o tom svedčí väčšina "tepla" v spodnej časti obrázku. Teplo má polohu z (c) a tvar z (b).}
